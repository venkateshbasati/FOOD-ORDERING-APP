...
import {  SetPriceDecrement, SetPriceIncrement, addItem,removeItems   } from "../utils/cartSlice";
      ...
      <div className="menu m-4">
        ...
        <ul>
          ...
          {rstMenu.map((items) => (
            ...
            <li  className="   font-medium  px-1  " key={items?.card?.info?.id}>
            ...
            <button  className="m-1 text-white rounded p-1 bg-slate-500 disabled:opacity-[0.45]" disabled={disableObj[ items?.card?.info?.id] ?? false  } onClick={()=>{ setDisableObj( (prev)=>  ({ ...prev, [items?.card?.info?.id]:true })) ; handleAddItems(items)} }>Add</button>
            <button  className="m-1 text-white rounded p-1 bg-slate-500 disabled:opacity-[0.45]" disabled={cartItems.length===0 ?? false  }onClick={()=>  handleRemoveItems(items)} > Remove</button>
            </div>
            </div>
